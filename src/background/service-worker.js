...
function jdHardReject(job) {
  ...
  var name = String(job.name || job.title || '');
  var all = name + ' ' + jd;
  // 1) 2027 届 / 毕业时间 2027 / 27届（2026 应届不可投）；岗位名含届别也算
  if (/2027\s*届|27\s*届|毕业时间[:：]?\s*2027|面向\s*2027|2027届|2027年|毕业时间[:：]?\s*20[0-9]{2}/.test(all)) {
    reasons.push('届别不符: 2027届');
  }
  // 2) JD 正文经验年限硬门槛：要求 1 年/2 年/3 年+ 且明确为任职要求（应届无法满足）
  //    "1-3年可投/可沟通"类表述 = 接受应届，放行；"要求1年以上/有N年工作经验" = 拦截
  var jdExp = jd.replace(/经验不限|不限经验/g, '');
  var freshFriendly = /接受.{0,4}(应届|无经验)|(应届|无经验).{0,6}(可投|可沟通|欢迎|优先)|在校生|实习.*转正/.test(jd);
  var strongExp = jdExp.match(/([一二三四五六七八九十两]|[1-9])\s*年(?:以上|及以上|工作经验|开发经验|相关工作经验)/);
  if (strongExp && !freshFriendly) {
    reasons.push('JD经验硬门槛: ' + strongExp[0]);
  } else {
    var m = jd.match(/(?:任职要求|岗位要求|职位要求|基本要求|我们需要你|我们希望你是|岗位要求[:：]?)[^。]{0,120}?([一二三四五六七八九十两]|[1-9])\s*(?:年|年以上|年及以上|年工作经验|年开发经验|年相关工作经验)/);
    if (m && !freshFriendly) reasons.push('JD经验硬门槛: ' + m[0].slice(0, 30));
  }
  // 3) 学历硬门槛：985/211/双一流/硕士及以上等（本科不可投）
  if (/985\s*(?:本科|院校|学校)?以上|211\s*(?:本科|院校|学校)?以上|双一流\s*以上|硕士及以上|研究生及以上|博士|硕士学历|仅限.{0,3}(985|211)|C9\s*(?:本科)?以上|985\s*本科/.test(jd)) {
    reasons.push('学历硬门槛(JD): 硕士/985');
  }
  // 4) 代招/外包/驻场识别（岗位挂公司名义但实际外包）
  var outsourceTerms = ['代招公司', '劳务派遣', '外包员工', '驻场', '客户方', '外包岗位', '外包性质', '外派', '派遣', '猎头顾问', '常驻'];
